import { collect_by_prefix, get_most_recent_message, get_nth_most_recent_message, register_command, SlashCommand } from "./collector.js";
...
collect_by_prefix("oexre", async (m, cont) => {
    ...
    let source = m.reference ? await m.fetchReference() : get_nth_most_recent_message(m.channel.messages, offset_i);
...
register_command(new SlashCommand().setDMPermission(true).setDescription("Replace all RegExp matches from message content with a template. Groups are mapped to &0, &1, ...")
    ...
    .addIntegerOption(i => i.setRequired(false).setMinValue(0).setName("offset").setDescription("The n-th to last message to use as the source. Default: 1 (Most recent message).")),
    async i => {
        await i.channel!.messages.fetch();
        let source = get_nth_most_recent_message(i.channel!.messages, (i.options.getInteger("offset") ?? 1) - 1);
