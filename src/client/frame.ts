...
collect(async (m) => {
    if(m.author.id == KARUTA_ID
        ...
        m.react("🖼");
        m.react("💰");
        const collector = m.createReactionCollector({ filter: (react, user) => react.emoji.name == "🖼" && user.id != m.client.user?.id, time: 30 * 1000, max: 1, maxEmojis: 1 });

        const msg = MessageHandler.as_message_reply(m);
        collector.on("collect", () => {
            hook_message_updates(m, (_) => {
                const image = m.embeds[0].image ?? m.embeds[0].thumbnail;
                image && msg.send({
