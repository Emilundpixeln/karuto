...
function guesses(c: Card): Array<string> {
    const url = klu_data?.[c.series]?.[c.name];
    if(!url) return [];
...
const tracked_messages: Map<string, Track> = new Map();
...
collect2(message => message.author.id == KARUTA_ID
    ...
    && (message.embeds[0].description.startsWith("Cards carried by")
        ...
        || message.embeds[0].title.startsWith("Character Results")), message => {
            ...
            const collecter = message.createReactionCollector({ filter: (reac) => reac.emoji.name == "🔎", dispose: true });
            ...
            collecter.on("dispose", async (reac) => {
                ...
                const track = tracked_messages.get(message.id);
                if(!track) return;
                track.deleter?.();
