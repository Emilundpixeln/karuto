import { collect, collect_by_prefix, collect_transition, get_most_recent_message, register_command, SlashCommand } from "./collector.js";
...
const run_solve = async (args: string[]) => {
    const path = process.env.DATESOLVER_PATH;
    if(!path) {
        console.error("Datesolver path not set!");
        return undefined;
    }
    const { stdout } = await execFileP(path, args);
    ...
    const board_lines = lines.slice(1, 16);
    return {
        board_lines,
...
const solve = async (author: string, url_or_key: string, get_ring: boolean, my_message: MessageHandler) => {
    const is_img = url_or_key.includes("https://dhp5ttvnehc80.cloudfront.net/");
    ...
    try {
        ...
        const vals_or_undefined = await run_solve(args);
        if(vals_or_undefined == undefined) {
            return my_message.send("Something went wrong...");
        }
        let vals = vals_or_undefined;
        ...
        if(board_has_ring && !path_has_ring && get_ring) {
            const force_vals = await run_solve(["-force_ring", "-key", vals.exchange_key]);
        ...
        const text = `${vals.path.trim().split(" ")
        ...
        const board_text = vals.board.map((line, y) => line.split("")
            .map((c, x) => y == 14 && x == 5 ? to_emoji(vals.car.includes("left") ? "left" : "right") : to_emoji(c)).join(" ")).join("\n");

        const row = new MessageActionRow();
        const target = "datesolve button"; //Math.random().toString().padEnd(19, "0").slice(2);
        if(get_ring)
            row.addComponents(
                new MessageButton()
                    .setCustomId(target)
...
collect_by_prefix("odate", async (m, cont) => {
    let url = undefined;
    if(m.reference) {
        const msg = await m.fetchReference();
...
register_command(new SlashCommand().setName("date").setDescription("solve a date"), (i) => {
    ...
    const msg = i.channel ? get_most_recent_message(i.channel.messages, (message) => {
        ...
        const visitor = /<@(\d+)>/g.exec(message.embeds[0].description);
...
const whereami = (my_msg: MessageHandler, inp_time: number, messages_for_search: MessageManager) => {
    const msg = get_most_recent_message(messages_for_search, (message) => message.author.id == messages_for_search.client.user?.id && message.embeds.length > 0 && !!message.embeds[0].title
...
register_command(new SlashCommand().setName("whereami").setDescription("Find remaining steps in a date")
    .addIntegerOption(option => option.setName("time").setDescription("Remaining Time ⌛").setMinValue(0).setMaxValue(100).setRequired(true)), (i) => {
        ...
        i.channel && whereami(MessageHandler.as_interaction_command_reply(i), i.options.getInteger("time", true), i.channel.messages);
    });

collect_by_prefix("owhereami", (user_message, cont) => {
    const time = Number.parseInt(cont);
...
collect_transition(async (message, old_message) => {
    ...
    const date_ask_regex = /You are about to spend `\d` Energy to ask \*\*.*\*\* out on a date.\nBased on your \*\*Affection Rating\*\*, there is a `.*` chance they will accept./;
