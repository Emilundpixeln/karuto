...
import { character_strs, series_strs, wl_data, wl_data_too_new } from "./shared/klu_data.js";
...
let set_ping_data = (client: Client, send: number, receive: number) => {
    ...
    let api_availability = api_available() ? "🪐" : "";
...
let do_ping = async (m: MessageHandler, created_timestamp: number) => {
    let begin = Date.now();
    let mymsg = await m.send("Pinging...");
...
let do_get_num_id = (m: MessageHandler, id: string) => {
    let num = encode_card_id(id);
    m.send(num != -1 ? `Card \`${id}\` has id \`${num}\`.` : "Not a valid card id!");
}

let do_from_num_id = (m: MessageHandler, id: string | number) => {
...
register_command(new SlashCommand().setDescription("Convert a card id like `xth0sj` to a number").setName("get_numeric_id")
    .addStringOption(e => e.setDescription("The Id").setName("id").setMinLength(1).setMaxLength(10).setRequired(true)), (i) => {
        let id = i.options.getString("id", true);
...
register_command(new SlashCommand().setDescription("Convert a numeric id to a card id like `xth0sj`").setName("from_numeric_id")
    .addIntegerOption(e => e.setDescription("The numeric id").setName("id").setMinValue(1).setRequired(true)), (i) => {
        let id = i.options.getInteger("id", true);
...
let do_query = (m: MessageHandler, query: string) => {
    let lowercase = query.toLowerCase();
    let serieses = series_strs.filter(s => s.toLowerCase().includes(lowercase));
    // [series, character, wl_data]
    let chars = Object.entries(wl_data).map(s => Object.entries(s[1]).filter(c => c[0].toLowerCase().includes(lowercase)).map(c => [s[0], ...c] as const)).flat();
    let text = serieses.map(v => `Series: **${v}**\n`).join("") + chars.map(v => `**${v[0]}** ${v[1]} \`♡${v[2].wl == wl_data_too_new ? "NEW" : v[2].wl}\``).join("\n");
